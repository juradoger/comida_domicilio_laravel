        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-orange-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-sm text-orange-500 mt-2">Pedidos realizados hoy <span class="inline-block ml-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-green-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-sm text-green-500 mt-2">Ingresos de hoy (bolivianos) <span class="inline-block ml-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-blue-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-sm text-blue-500 mt-2">Usuarios registrados <span class="inline-block ml-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></span></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-red-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-sm text-red-500 mt-2">Productos con poco stock <span class="inline-block ml-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></span></p>
                </div>
                ...
                <div class="w-full h-full flex items-end justify-between space-x-2">
                    ...
                    @foreach($meses as $index => $mes)
                        <div class="flex flex-col items-center">
                            <div class="bg-blue-500 rounded-t-sm w-6" style="height: {{ ($valores[$index] / $max) * 100 }}%"></div>
                ...
                <div class="w-full max-w-xs">
                    <div class="relative h-64 w-64 mx-auto">
                        <svg viewBox="0 0 36 36" class="w-full h-full">
                            ...
                            @php
                                $estados = [
                                    'pendiente' => ['color' => '#F59E0B', 'porcentaje' => $estadoPedidos['pendiente'] ?? 0],
                                    'en_preparacion' => ['color' => '#8B5CF6', 'porcentaje' => $estadoPedidos['en_preparacion'] ?? 0],
                                    'en_camino' => ['color' => '#3B82F6', 'porcentaje' => $estadoPedidos['en_camino'] ?? 0],
                                    'entregado' => ['color' => '#10B981', 'porcentaje' => $estadoPedidos['entregado'] ?? 0],
                                    'cancelado' => ['color' => '#EF4444', 'porcentaje' => $estadoPedidos['cancelado'] ?? 0]
                                ];
                            ...
                            @foreach($estados as $estado => $info)
                                ...
                                @if($porcentaje > 0)
                                    <path d="M 18 18 L {{ $x1 }} {{ $y1 }} A 16 16 0 {{ $largeArcFlag }} 1 {{ $x2 }} {{ $y2 }} Z" 
                                          fill="{{ $info['color'] }}" />
                                @endif
                            @endforeach
            ...
            <div class="mt-4 flex flex-wrap justify-center gap-2">
                @foreach($estados as $estado => $info)
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs" 
                        ...
                        <span class="w-2 h-2 rounded-full mr-1" style="background-color: {{ $info['color'] }}"></span>
            ...
            <div class="h-64 flex items-center justify-center">
                <div class="w-full h-full">
                    ...
                    <svg viewBox="0 0 100 100" class="w-full h-full" preserveAspectRatio="none">
                        ...
                        <!-- Área bajo la curva -->
                        <polyline points="0,100 {{ $puntosStr }} 100,100" 
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($productosMasVendidos as $detalle)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $detalle->producto->categoria->nombre }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $detalle->total_vendido }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">Bs {{ number_format($detalle->total_vendido * $detalle->producto->precio, 2) }}</div>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
