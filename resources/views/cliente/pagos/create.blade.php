...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('cliente.pagos.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-4 rounded transition duration-300">
                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p><span class="font-medium">Total a pagar:</span> <span class="font-bold">S/ {{ number_format($pedido->total, 2) }}</span></p>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('cliente.pagos.store') }}" class="space-y-6">
                    ...
                    @else
                        <div class="mb-4">
                            ...
                            <select name="pedido_id" id="pedido_id" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('pedido_id') border-red-500 @enderror">
                                ...
                                @foreach($pedidos as $pedido)
                                    <option value="{{ $pedido->id }}" data-monto="{{ $pedido->total }}" {{ old('pedido_id') == $pedido->id ? 'selected' : '' }}>
                                        Pedido #{{ $pedido->id }} - S/ {{ number_format($pedido->total, 2) }} - {{ ucfirst(str_replace('_', ' ', $pedido->estado)) }}
                                    </option>
                                @endforeach
                            </select>
                            @error('pedido_id')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="monto" id="monto" step="0.01" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('monto') border-red-500 @enderror" value="{{ old('monto') }}" readonly>
                            @error('monto')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="metodo_pago" id="metodo_pago" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('metodo_pago') border-red-500 @enderror">
                    ...
                    <div class="mb-4">
                        <label for="referencia" class="block text-gray-700 text-sm font-bold mb-2">Referencia de Pago (opcional):</label>
                        <input type="text" name="referencia" id="referencia" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('referencia') }}" placeholder="Número de operación, últimos dígitos de tarjeta, etc.">
                    </div>
                    
                    <div class="mb-4">
                        <label for="comentarios" class="block text-gray-700 text-sm font-bold mb-2">Comentarios (opcional):</label>
                        <textarea name="comentarios" id="comentarios" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Información adicional sobre el pago">{{ old('comentarios') }}</textarea>
                    </div>
