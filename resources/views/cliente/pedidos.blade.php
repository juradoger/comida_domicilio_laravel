                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gradient-to-r from-orange-500 to-red-600">
                        <tr>
                            ...
                            <th class="py-3 px-4 text-left text-xs font-medium text-white uppercase tracking-wider">Productos
                            </th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-white uppercase tracking-wider">Total
                            </th>
                        ...
                        @foreach ($pedidos as $pedido)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $pedido->created_at->format('d/m/Y H:i') }}</td>
                                <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        @if ($pedido->estado == 'entregado' && !$pedido->calificacion)
                                            <a href="{{ route('cliente.calificaciones.create', ['pedido_id' => $pedido->id]) }}"
                                                class="text-yellow-600 hover:text-yellow-900">Calificar</a>
                                        @endif

                                        @if ($pedido->estado == 'pendiente')
                                            <form action="{{ route('cliente.pedidos.cancelar', $pedido->id) }}"
                                        ...
                                        @if (!$pedido->pago && ($pedido->estado == 'pendiente' || $pedido->estado == 'en_proceso'))
                                            <a href="{{ route('cliente.pagos.create', ['pedido_id' => $pedido->id]) }}"
                                                class="text-green-600 hover:text-green-900">Pagar</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
