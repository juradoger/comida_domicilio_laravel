        ...
        <div class="space-y-3">
            @if ($this->getNotificaciones()->count() > 0)
                @foreach ($this->getNotificaciones() as $notificacion)
                    <div
                        class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 {{ !$notificacion->leido ? 'border-l-4 border-l-orange-500' : '' }}">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">
                                            @switch($notificacion->tipo)
                                                ...
                                                @default
                                                    <span
                                                        class="text-blue-600">{{ ucfirst(str_replace('_', ' ', $notificacion->tipo)) }}</span>
                                            @endswitch
                                        </span>
                                    </div>
                                    <span
                                        class="text-sm text-gray-500">{{ $notificacion->fecha_envio->diffForHumans() }}</span>
                                </div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-3 text-xs text-gray-600">
                                        <div class="grid grid-cols-2 gap-2">
                                            @if (isset($notificacion->datos_adicionales['producto_nombre']))
                                                <div><span class="font-medium">Producto:</span>
                                                    {{ $notificacion->datos_adicionales['producto_nombre'] }}</div>
                                            @endif
                                            @if (isset($notificacion->datos_adicionales['stock_actual']))
                                                <div><span class="font-medium">Stock actual:</span>
                                                    {{ $notificacion->datos_adicionales['stock_actual'] }} unidades
                                                </div>
                                            @endif
                                        </div>
                                    </div>
