    ...
    <div class="space-y-4">
        @if($this->getNotificaciones()->count() > 0)
            @foreach($this->getNotificaciones() as $notificacion)
                <div class="p-4 bg-white rounded-lg shadow-sm border {{ !$notificacion->leido ? 'border-orange-200 bg-orange-50' : 'border-gray-200' }}">
                            ...
                            @if($notificacion->datos_adicionales)
                                <div class="mt-2 text-xs text-gray-600">
                                        ...
                                        <ul class="mt-1 ml-4 list-disc">
                                            @foreach($notificacion->datos_adicionales['productos'] as $producto)
                                                @if(is_array($producto))
                                                    <li>{{ $producto['nombre'] }} ({{ $producto['stock_actual'] }} unidades)</li>
                                                @else
                                                    <li>{{ $producto }}</li>
                                                @endif
                                            @endforeach
                                    ...
                                    @if(isset($notificacion->datos_adicionales['stock_actual']))
                                        <p><strong>Stock actual:</strong> {{ $notificacion->datos_adicionales['stock_actual'] }} unidades</p>
                                    @endif
                                </div>
                            @endif
                        </div>
