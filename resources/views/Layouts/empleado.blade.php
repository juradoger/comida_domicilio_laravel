...
<head>
    ...
    <title>@yield('title', 'Panel Empleado')</title>
    <!-- Tailwind CSS CDN -->
    ...
    <header class="navbar-gradient text-white shadow-md">
        <div class="container mx-auto flex justify-between items-center py-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-2xl font-extrabold text-gradient">Panel Empleado</span>
            </div>
            <nav class="flex gap-6 items-center">
                <a href="{{ route('empleado.clientes') }}" class="hover:underline font-semibold transition">Clientes</a>
                <a href="{{ route('empleado.repartidores') }}" class="hover:underline font-semibold transition">Repartidores</a>
                <a href="{{ route('empleado.repartos.index') }}" class="hover:underline font-semibold transition">Repartos</a>
                ...
                <div class="relative group">
                    <button class="flex items-center gap-1 hover:underline font-semibold transition">
                        ...
                        <span class="bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center absolute -top-1 -right-1">{{ Auth::user()->notificacionesNoLeidas()->count() }}</span>
                    </button>
                    <div class="absolute right-0 mt-2 w-72 bg-white rounded-md shadow-lg py-1 z-10 hidden group-hover:block">
                        ...
                        @if(Auth::user()->notificaciones()->count() > 0)
                            @foreach(Auth::user()->notificaciones()->latest()->take(5)->get() as $notificacion)
                                <div class="px-4 py-2 border-b border-gray-100 {{ $notificacion->leida ? 'bg-white' : 'bg-orange-50' }}">
                            ...
                            @endforeach
                            <a href="{{ route('empleado.notificaciones.index') }}" class="block px-4 py-2 text-center text-orange-600 hover:underline">Ver todas</a>
                        @else
                ...
                @auth
                    <div class="relative group ml-4">
                        <button class="flex items-center gap-2 px-4 py-2 rounded btn-gradient text-white font-bold shadow hover:scale-105 transition">
                            {{ Auth::user()->nombre }}
    ...
    <footer class="navbar-gradient text-white mt-10 shadow-inner">
        <div class="container mx-auto px-4 py-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div class="flex flex-col gap-2">
                ...
                <span class="text-sm">Gestión de clientes, repartidores, productos y reportes.</span>
            </div>
        ...
        <div class="text-center text-xs py-2 bg-opacity-30 bg-black">&copy; {{ date('Y') }} Panel Empleado. Todos los derechos reservados.</div>
    </footer>
    @stack('scripts')
</body>
</html>