...
<head>
    ...
    <style>
        ...
        .sidebar-link { @apply flex items-center gap-2 py-2 px-4 rounded-lg transition-all; }
        .sidebar-link:hover { @apply bg-orange-100 text-orange-600; }
        .sidebar-link.active { @apply bg-orange-500 text-white; }
    </style>
    @stack('styles')
</head>
<body class="bg-gray-100 text-gray-800 min-h-screen flex flex-col">
    ...
    <header class="navbar-gradient text-white shadow-md">
        <div class="container mx-auto flex justify-between items-center py-4">
            ...
            <nav class="flex gap-6 items-center">
                ...
                <a href="{{ route('admin.pedidos.index') }}" class="hover:underline font-semibold transition">Pedidos</a>
                <a href="{{ route('admin.clientes.index') }}" class="hover:underline font-semibold transition">Clientes</a>
                <a href="{{ route('admin.empleados.index') }}" class="hover:underline font-semibold transition">Empleados</a>
                <a href="{{ route('productos.index') }}" class="hover:underline font-semibold transition">Productos</a>
                <a href="{{ route('categorias.index') }}" class="hover:underline font-semibold transition">Categorías</a>
                <div class="relative group">
                    <button class="flex items-center gap-1 hover:underline font-semibold transition">
                        Más <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10 hidden group-hover:block">
                        <a href="{{ route('admin.pagos.index') }}" class="block px-4 py-2 text-gray-800 hover:bg-orange-100">Pagos</a>
                        <a href="{{ route('admin.calificaciones.index') }}" class="block px-4 py-2 text-gray-800 hover:bg-orange-100">Calificaciones</a>
                        <a href="{{ route('admin.notificaciones.index') }}" class="block px-4 py-2 text-gray-800 hover:bg-orange-100">Notificaciones</a>
                        <a href="{{ route('admin.estadisticas') }}" class="block px-4 py-2 text-gray-800 hover:bg-orange-100">Estadísticas</a>
                    </div>
                </div>
                @auth
    ...
    <footer class="navbar-gradient text-white mt-10 shadow-inner">
        ...
        <div class="text-center text-xs py-2 bg-opacity-30 bg-black">&copy; {{ date('Y') }} Panel Administrador. Todos los derechos reservados.</div>
    </footer>
    @stack('scripts')
</body>
</html>